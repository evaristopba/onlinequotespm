...
export default function BarcodeScanner({ onScan, onClose }) {
  ...
  useEffect(() => {
    ...
    Html5Qrcode.getCameras().then(cameras => {
      if (cameras.length === 0) {
        setErro('Nenhuma câmera encontrada. Use o campo manual.')
        setCarregando(false)
        return
      }
      const camera = cameras.find(c => c.label.toLowerCase().includes('back') || c.label.toLowerCase().includes('traseira'))
      ...
      setErro('Erro ao listar câmeras: ' + err.message)
      setCarregando(false)
    })
